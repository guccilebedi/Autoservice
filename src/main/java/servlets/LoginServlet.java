    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) // метод post
        ...
        try {
            if (nonNull(session) && nonNull(session.getAttribute("login")) && nonNull(session.getAttribute("password"))) { // если пользователь уже вошёл в систему, перенаправить его на нужную страницу
                ...
                if (loginResult.get(0) != null) { // если пользователь найден
                    request.getSession().setAttribute("password", password);
                    request.getSession().setAttribute("login", userName);
                    request.getSession().setAttribute("role", loginResult.get(0));
                    request.getSession().setAttribute("fullName", loginResult.get(1));
                    moveToMainPage(request, response, (Role) loginResult.get(0)); // добавить в сессию аттрибуты и перенаправить на нужнкю страницу
